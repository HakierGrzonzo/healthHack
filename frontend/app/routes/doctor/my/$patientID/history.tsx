...
export const loader: LoaderFunction = async ({params}) => {
  ...
  const saneConditions: Condition[] =JSON.parse(conditions).entry.map((e) => e.resource)
  const symptoms = getSymptoms(saneConditions.map(c => c.code?.text).filter(v => v))
  const prediction = symptoms.length > 0 ? (await SymptomPredictClient.default.predictPost({
    symptoms
  })) : null
    ...
    <Box sx={{
        ...
        {prediction &&
          <Alert severity="info">
            ...
            likely that the patient might be suffering from <strong>{prediction}</strong>.
          </Alert>
        }
